  ...
  <div id="personal_securitySetting">
    <div class="panel">
      ...
      <div class="panel-body"
        ...
        <el-form class="app_form" :model="passwordForm" ref="passForm" :rules="passRules" label-width="90px">
          <el-form-item :label="$t('securitySetting.label.old_pass')" prop="oldPassword" require>
            <el-input type="password" v-model="passwordForm.oldPassword"
                      :placeholder="$t('securitySetting.label.enter_old_pass')"
                      :maxlength=30></el-input>
          </el-form-item>
          <el-form-item :label="$t('securitySetting.label.new_pass')" prop="newPassword" require>
            <el-input type="password" v-model="passwordForm.newPassword"
                      :placeholder="$t('securitySetting.label.enter_new_pass')"
                      :maxlength=30></el-input>
          </el-form-item>
          <el-form-item :label="$t('securitySetting.label.repeat_pass')" prop="newPassword2" require>
            <el-input type="password" v-model="passwordForm.newPassword2"
  ...
  export default {
    ...
    methods: {
      initRules() {
        ...
        this.passRules = {
          oldPassword: [
            {required: true, message: this.$t('common.msg.enter_old_pass'), trigger: 'blur'},
            {min: 6, max: 20, message: this.$t('securitySetting.msg.organ_len_20')},
            {validator: Validate.trim, trigger: "blur"}
          ],
          newPassword: [
            {required: true, message: this.$t('common.msg.enter_new_pass'), trigger: 'blur'},
            {min: 6, max: 20, message: this.$t('securitySetting.msg.organ_len_20')},
            {validator: Validate.trim, trigger: "blur"}
          ],
          newPassword2: [
            {required: true, message: this.$t('common.msg.enter_repeat_pass'), trigger: 'blur'},
            {min: 6, max: 20, message: this.$t('securitySetting.msg.organ_len_20')},
            {validator: Validate.trim, trigger: "blur"},
      ...
      save() {
        ...
        self.$refs.passForm.validate(function (valid) {
          if (valid) {
            ...
            self.$store.dispatch("modifyPassword", {
