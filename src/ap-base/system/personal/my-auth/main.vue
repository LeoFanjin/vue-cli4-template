    ...
    <div class="panel-body"
      ...
      <ul class="list">
        ...
        <li v-for="item in authList">
          <div class="auth-name" style="width: 45%;text-align: center">{{ item.permissionName }}</div>
...
//          {permissionName:"超级管理员", type:"user"}
    ...
    created() {
      if (this.$store.state.loginInfo) {
        this.organName = this.$store.state.loginInfo.organ? this.$store.state.loginInfo.organ.organName : "";
      }
      this.init();
    },
    methods: {
      init() {
        ...
        self.$store.dispatch('findMyAuth').then(function (data) {
