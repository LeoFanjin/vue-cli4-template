    ...
    <div class="panel">
      ...
      <div class="panel-body"
        ...
        <div class="current_head">
          <ss_picture :src="head.src" @edit="editHead" @remove="removeHead"></ss_picture>
        </div>
...
//      shear
    ...
    data() {
      return {
        ...
        defaultImg: [
          {src: "/dist/vendor/image/head/default/head.jpg", active: false, id: 1},
          {src: "/dist/vendor/image/head/default/01.png", active: false, id: 2},
          {src: "/dist/vendor/image/head/default/head.jpg", active: false, id: 3},
          {src: "/dist/vendor/image/head/default/head.jpg", active: false, id: 4},
          {src: "/dist/vendor/image/head/default/head.jpg", active: false, id: 5}
        ],
        status: {
          isCanSave: false
        },
    ...
    created() {
      this.head.cropBtn = {ok: this.$t('common.label.save'), 'cancel': this.$t('common.label.cancel')};
      this.initHead();
      this.init();
    },
    methods: {
      init() {
        ...
        self.$store.dispatch('findUserHead').then(function (data) {
       ...
       */
      editHead() {
        let _input = $("#personal_head .g-core-image-upload-btn input");
      ...
      removeHead() {
        ...
        if (self.head.src === self.$store.state.default_head_img) {
          ...
          self.$confirm(self.$t('common.label.sure_remove'), self.$t('common.label.prompt'), {
            ...
            self.$store.dispatch("removeUserHead", {}).then(function () {
              self.$notify.success(self.$t('userHead.msg.head_update_success'));
              self.initHead();
              self.$store.dispatch("setUserHead", '');

            }).catch(function (message) {
              self.$notify.error((message && typeof message === "string") ? message : self.$t('common.msg.delete_fail'));
            });
          }).catch(function () {
          });
        }
      ...
      uploadedSuccess(res) {
        ...
        if (!_.isEmpty(res)) {
          Ajax.dealSuccess(res).then(function (data) {
            ...
            self.$notify.error((error && typeof error === 'string') ? error : '上传失败');
          });
        } else {
          self.$message("上传失败");
        }
