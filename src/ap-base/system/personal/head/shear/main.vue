    ...
    <div class="edit_head">
      ...
      <div class="m_left" v-if="!status.isTailoring">
        <div class="m_container">
          ...
          <div class="m_opt">
            <button class="btn btn-default btn-sm" @click="openFile">选择图片</button>
        ...
        <div class="m_container">
          ...
          <ul class="m_default_img">
            <li v-for="item in defaultImg" @click="changeCurrentImg(item.id)" :class="{active: item.active}"><img :src="item.src"></li>
          </ul>
          <div class="m_opt"><button class="btn btn-default btn-sm" :disabled="!status.isCanSave" @click="saveSystemHead">保存头像</button></div>
        </div>
      </div>
      <!-- 选择剪切图片 -->
      <div class="m_left" v-if="status.isTailoring">
        <div class="pre-big-content" style="width: 280px;height: 280px;border: 1px solid @app-assist-color-lightblue;">
          <div class="pre-big" :style="{'width': big.width + 'px', 'height': big.height + 'px'}">
            <div v-if="move.show" class="bg-top" :style="topStyle"></div>
            <div v-if="move.show" class="bg-left" :style="leftStyle"></div>
            <div v-if="move.show" id="selector" style="left: 0; top: 0;" :style="centerStyle" class="selector" @mouseup="mouseup" @mouseout="mouseup" @mousedown="mousedown" @mousemove="mouseMove"></div>
        ...
        <div class="opt">
          <button class="btn btn-default btn-sm" @click="startUpload">保存头像</button>
          <button class="btn btn-default btn-sm" @click="openFile">重新选择</button>
      ...
      <div class="preview">
        ...
        <p class="describe">你上传的图片会自动生成2种尺寸,请注意小尺寸的头像是否清晰</p>
        <div class="pre-default">
          <img v-show="isShowDefaultImg" src="/dist/vendor/image/head/head.jpg" alt="" style="width: 140px;height: 140px;">
          <img v-show="!isShowDefaultImg" id="default-img" :src="head.src" :style="normalImgStyle">
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      topStyle: {
        get() {
          return {position: "absolute", top: 0, left: 0, right: 0, bottom: this.big.height - this.big.center.top + 'px'};
        }
      },
      // big -> left style
      leftStyle: {
        get() {
          return {position: "absolute", top: this.big.center.top + 'px', left: 0, right: this.big.width - this.big.center.left + 'px', bottom: this.big.height - this.big.center.top - this.big.center.height + 'px'};
        }
      },
      // big -> center style
      centerStyle: {
        get() {
          return {position: "absolute", top: this.big.center.top + 'px', left: this.big.center.left + 'px', width: 140 + 'px', height: 140 + 'px'};
        }
      },
      // big -> right style
      rightStyle: {
        get() {
          return {position: "absolute", top: this.big.center.top + 'px', left: this.big.center.left + this.big.center.width + 'px', right: 0, bottom: this.big.height - this.big.center.top - this.big.center.height + 'px'};
        }
      },
      // big -> bottom style
      bottomStyle: {
        get() {
          return {position: "absolute", top: this.big.center.top + this.big.center.height + 'px', left: 0, right: 0, bottom: 0};
        }
      },
      normalImgStyle: {
        get() {
          ...
          return {top: -this.big.center.top + 'px', left: -this.big.center.left + 'px', width: width + 'px', height: height + 'px'};
        }
      },
      smallImgStyle: {
        get() {
          ...
          if (this.status.isTailoring) {
            ...
            return {top: -(this.big.center.top / 8 * 2) + 'px', left: -(this.big.center.left / 8 * 2) + 'px', width: this.big.width / 8 * 2 + 'px', height: this.big.height / 8 * 2 + 'px'};
          } else {
            return {top: 0, left: 0, width: this.small.width + 'px', height: this.small.width + 'px'};
          }
    ...
    methods: {
      ...
      addPreview(file, imgSrc) {
        ...
        if (imgSrc) {
          ...
          setTimeout(function () {
            ...
            if (localImg.offsetWidth > localImg.offsetHeight || localImg.offsetWidth === localImg.offsetHeight) {
              imgContainer.style.height = 280*(localImg.offsetHeight/localImg.offsetWidth) + "px";
              imgContainer.style.width = 280 + "px";
              localImg.style.width = 100 + "%";
              self.big.width = 280;
              self.big.height = 280*(localImg.offsetHeight/localImg.offsetWidth);
            }else {
              imgContainer.style.height = 280+ "px";
              imgContainer.style.width = 280*(localImg.offsetWidth/localImg.offsetHeight) + "px";
              localImg.style.height = 280 + "px";
              self.big.width = 280*(localImg.offsetWidth/localImg.offsetHeight);
              self.big.height = 280;
            }
          },500)
      ...
      moveSelector(e) {
        ...
        var x = e.clientX;
        var y = e.clientY;
        var left = parseInt(this.move.objX) + parseInt(x) - parseInt(this.move.mouseX);
        var top = parseInt(this.move.objY) + parseInt(y) - parseInt(this.move.mouseY);
      ...
      saveSystemHead() {
        var self = this;
        if (self.imageId) {
          self.$store.dispatch("saveSystemHead",{imageId:self.imageId}).then(function () {
