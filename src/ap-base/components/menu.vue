  ...
  <div class="app-side-nav">
    ...
    <div class="app-side-nav__nav">
      ...
      <ul :style="{'height':$store.state.min_height + 'px'}">
        <li v-for="item in getResource" v-if="!item.model && !item.isPersonal"
            :class="{active: item.isActive, open: item.open}">
          <a href="javascript:void(0)" :title="item.name" @click="toggleMenu(item.id, false)">
            <b v-if="!is_collapsed && item.children && item.children.length > 0 && !item.open"
               class="collapse-sign"><i class="icon iconfont icon-ap-thinarrow-right"></i></b>
            <b v-if="!is_collapsed && item.children && item.children.length > 0 && item.open"
               class="collapse-sign"><i class="icon iconfont icon-ap-thinarrow-down"></i></b>
          ...
          <transition name="slide" mode="out-in">
            <ul v-if="item.children && item.children.length > 0 && item.open"
                :style="{ 'left': is_collapsed?'90%': '100%'}">
              <li v-for="cItem in item.children" :class="{active: cItem.isActive}">
                <a href="javascript:void(0)" :title="cItem.name" @click="authFilter(cItem.path, cItem.path)">
                  <i v-if="cItem.icon" class="icon iconfont" :class="cItem.icon"></i>
  ...
  export default {
    ...
    created() {
      ...
      if (self.$store.state.resource && self.$store.state.resource.length === 0) {
        self.$store.dispatch('queryMyResources', {system_code: G.system_code, tree: 1}).then((data) => {
          var to = self.$route;
          ...
          if (data.length > 0) {
            ...
            if (G.homePage === to.path && G.homePage !== "/home" && !isContainsHome && data[0]) {
              to = {
                path: data[0],
                fullPath: data[0]
              };
    ...
    methods: {
      init() {
        var self = this;
        $('body').removeClass('app-nav-collapsed');
      ...
      closeNav() {
        $('body').addClass('app-nav-collapsed');
      ...
      toggleMenu(resourceId, event) {
        ...
        if (resourceId) {
          _.each(this.getResource, (resource) => {
            if (resource.children && resource.children.length > 0 && resourceId === resource.id) {
      ...
      activeMenu(fullPath, event) {
        ...
        if (fullPath && !event) {
          ...
          if ((!cResource || _.isEmpty(cResource) || !(fullPath === cResource.path)) || !event) {
            var path = '';

            _.each(this.getResource, (resource) => {
              if (resource.path && _s(fullPath).startsWith(resource.path)) {
                currentResource = {id: resource.id, name: resource.name, path: resource.path};
                path = resource.path;
            ...
            let getAllPath = function(routes, parentPath) {
              ...
              _.each(routes, function (route) {
                ...
                if (route.children) {
                  paths = _.union(paths, getAllPath(route.children, prefix + route.path));
                }
              });
